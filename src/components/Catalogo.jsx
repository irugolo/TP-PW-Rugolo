...
export default function Catalogo({ experiencias, onSeleccionar }) {
  ...
  return (
    <section id="experiencias" className="catalogo seccion" aria-labelledby="titulo-catalogo">
      <div className="seccion-cabecera">
        <div><p className="eyebrow">Un plan para cada compañía</p><h2 id="titulo-catalogo">¿Qué plan te pinta?</h2></div>
      ...
      <div className="filtros" role="group" aria-label="Filtrar experiencias por compañía">
        {filtros.map(opcion => <button key={opcion} aria-pressed={filtro === opcion} onClick={() => setFiltro(opcion)}>{opcion}</button>)}
      </div>
      ...
      <div className="grilla">{visibles.map(experiencia => <TarjetaExperiencia key={experiencia.id} experiencia={experiencia} onSeleccionar={onSeleccionar} />)}</div>
    </section>
  );
}
